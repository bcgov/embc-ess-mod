...
import { EvacuationFileHouseholdMember, RegistrantProfileSearchResult } from 'src/app/core/api/models';
...
export class HouseholdMemberService {
   ...
   */
  public createNavigationForRegistrant(value: string, memberDetails: EvacuationFileHouseholdMember) {
    if (value === 'Yes') {
      ...
      this.setProfileDetails(memberDetails.linkedRegistrantId);
      this.evacueeSessionService.securityQuestionsOpenedFrom = 'responder-access/search/essfile-dashboard';
  ...
  public setProfileDetails(id: string) {
    ...
    const profileReload = this.appBaseService?.appModel?.selectedProfile?.selectedEvacueeInContext === null;
  ...
  navigateForMultipleLink(essFile: EvacuationFileModel, multipleLinkRegistrants: MultipleLinkRegistrantModel, value) {
   ...
   * @param memberDetails household member
   */
  public singleMatchedRegistrantLink(memberDetails: EvacuationFileHouseholdMember, essFile: EvacuationFileModel): void {
    this.evacueeSessionService.securityQuestionsOpenedFrom = 'responder-access/search/essfile-dashboard';
    this.evacueeSessionService.fileLinkMetaData = {
      fileId: essFile.id,
      linkRequest: {
   ...
   */
  public sortByAuthenticationFactor(matchedProfiles: RegistrantProfileSearchResult[]): RegistrantProfileSearchResult[] {
    return matchedProfiles.sort((a, b) => (a.isAuthenticated === b.isAuthenticated ? 0 : a.isAuthenticated ? -1 : 1));
  }
   ...
   * @returns sorted array
   */
  public sortByVerificationFactor(matchedProfiles: RegistrantProfileSearchResult[]): RegistrantProfileSearchResult[] {
    return matchedProfiles.sort((a, b) => (a.status ? a.status.localeCompare(b.status) : -1));
  }

  public createMultipleRegistrantModel(memberDetails: EvacuationFileHouseholdMember): MultipleLinkRegistrantModel {
    return {
  ...
  public linkMemberProfile(memberDetails: EvacuationFileHouseholdMember, essFile: EvacuationFileModel) {
    if (this.essfileDashboardService.matchedProfiles.length === 1) {
      ...
      this.multipleMatchedRegistrantLink(this.createMultipleRegistrantModel(memberDetails), essFile);
    }
  }
}
