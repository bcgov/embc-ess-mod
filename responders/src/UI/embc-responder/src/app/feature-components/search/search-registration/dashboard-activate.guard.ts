...
export class DashboardActivateGuard {
  ...
  private canActivateRoute(registrantProfileId: string, selectedFileId: string): boolean {
    if (
      this.appBaseService?.appModel?.selectedUserPathway === SelectedPathType.remoteExtensions ||
      this.appBaseService?.appModel?.selectedUserPathway === SelectedPathType.caseNotes
    ) {
      return (
        selectedFileId !== null &&
