...
public class EvacuationsController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [Authorize]
    public async Task<ActionResult<IEnumerable<EvacuationFile>>> GetCurrentEvacuations(CancellationToken ct)
    {
        try
        {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [Authorize]
    public async Task<ActionResult<IEnumerable<EvacuationFile>>> GetPastEvacuations(CancellationToken ct)
    {
        var userId = currentUserId;
        var files = await evacuationSearchService.GetFiles(userId, [ESS.Shared.Contracts.Events.EvacuationFileStatus.Archived, ESS.Shared.Contracts.Events.EvacuationFileStatus.Completed], ct);
    ...
    [Authorize]
    public async Task<ActionResult<RegistrationResult>> UpsertEvacuationFile(EvacuationFile evacuationFile, CancellationToken ct)
    {
...
/// </summary>
public record AnonymousRegistration
{
    [Required]
...
public record LodgingHotelSupport : Support
{
...
public record LodgingBilletingSupport : Support
{
    public override SupportCategory Category => SupportCategory.Lodging;
...
public record LodgingGroupSupport : Support
{
    public override SupportCategory Category => SupportCategory.Lodging;
...
public record LodgingAllowanceSupport : Support
{
    public override SupportCategory Category => SupportCategory.Lodging;
