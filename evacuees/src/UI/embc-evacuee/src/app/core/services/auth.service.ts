...
export class AuthService {
  ...
  public login(returnPath: string): void {
    ...
    if (
      ...
      environment.hasOwnProperty(loginAsPropertyName)
    ) {
  ...
  public getToken(): Observable<string> {
    ...
    return this.token
      ? of(this.token)
      : this.loginService.loginToken().pipe(
          switchMap((token) => {
            this.token = token;
  ...
  public refreshToken(): Observable<boolean> {
    return this.loginService.loginRefreshToken().pipe(
      switchMap((token) => {
        // update token
        this.token = token;
        return of(true);
      ...
      retryWhen((errors) =>
        errors.pipe(delay(this.httpRetryDelay), take(this.httpRetryNumber))
      ),
      catchError((error) => {
        console.error(error);
        if (error instanceof HttpErrorResponse && error.status === 401) {
