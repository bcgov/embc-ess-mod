...
export class AllowNavigationGuard implements CanActivate {
    ...
    constructor(
        ...
        private conflictService: ConflictManagementService, private evacuationFileDataService: EvacuationFileDataService,
            ...
            if (!exists && state.url === '/verified-registration') {
                ...
                if (state.url === '/verified-registration/conflicts' && this.conflictService.getCount() === 0) {
                    ...
                    this.profileService.getConflicts().subscribe(conflicts => {
                        ...
                        } else if ((state.url === '/verified-registration/conflicts' && this.conflictService.getCount() !== 0) ||
                            state.url.match('/dashboard/')) {
                            this.profileService.getProfile();
                            if (state.url.match('/dashboard/current/') && this.evacuationFileDataService.essFileNumber === undefined) {
                                this.router.navigate(['/verified-registration/dashboard/current']);
                            } else if (state.url.match('/dashboard/past/') && this.evacuationFileDataService.essFileNumber === undefined) {
